...
public class GameTest {
    ...
    public void createDecksTest() {
        assertEquals("Incorrect amount of decks created", 4, game.getDecks().size());
    }

    @Test
    public void createPlayersTest() {
        assertEquals("Incorrect amount of players created", 4, game.getPlayers().size());
    }

    @Test
    public void validatePackTest() {
        assertTrue("Valid pack location and contents", game.validPack("test_pack_0.txt"));
        assertFalse("Invalid pack location", game.validPack("doesNotExist.txt"));
        assertFalse("Invalid pack contents: Multiple values per line", game.validPack("test_pack_1.txt"));
        assertFalse("Invalid pack contents: Non integers", game.validPack("test_pack_2.txt"));
        assertFalse("Invalid pack contents: Negative integers", game.validPack("test_pack_3.txt"));
        assertFalse("Invalid pack contents: Wrong amount of integers", game.validPack("test_pack_4.txt"));
    }
    ...
    public void distributedCardsTest() {
        ...
        CardDeck d1 = (CardDeck) game.getDecks().get(0);

        // getting the size of players and decks hand
        int playerHandSize = p1.getPlayerHandList().size();
        int deckHandSize = d1.getDeckCards().size();

        // Getting the value of the 4th card in their hands
        Card playerHandCard = (Card) p1.getPlayerHandList().get(3);
        Card deckHandCard = d1.getDeckCards().get(3);
        int playerCardValue = playerHandCard.getValue();
        int deckCardValue = deckHandCard.getValue();
